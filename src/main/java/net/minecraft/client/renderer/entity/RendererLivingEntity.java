...
//import me.superskidder.fpsmaster.connection.Service;
import top.fpsmaster.FPSMaster;
import top.fpsmaster.modules.misc.CheaterDetector;
import top.fpsmaster.modules.render.NameTag;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void renderName(T entity, double x, double y, double z) {
        ...
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z) || (NameTag.TOGGLE && !entity.isInvisible())) {
            ...
            if (this.canRenderName(entity)) {
                ...
                if (d0 < (double) (f * f)) {
                    ...
                    if (entity instanceof EntityPlayer) {
                        if (CheatDetection.get(entity.getUniqueID()).hacks && ((CheaterDetector) FPSMaster.INSTANCE.moduleManager.modules.get("CheaterDetector")).mark.getValue()) {
                            s = "\247c\247l[CHEATER] \247r" + s;
                        }
                    }
                    GlStateManager.alphaFunc(516, 0.1F);
...
//                        if (Service.isClientMate(entity.getName())) {
...
//                            worldrenderer.pos(i + 9, 8.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
//                            worldrenderer.pos(i + 9, -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
//                            tessellator.draw();
//                            GlStateManager.enableTexture2D();
//                            GlStateManager.depthMask(true);
//                            绘制clientmate
//                            fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2 + 8, 0, 553648127);
//                            RenderUtil.drawImage(new ResourceLocation("client/logo.png"), -fontrenderer.getStringWidth(s) / 2, 0, 8, 8, 100);
//                        } else {
                        worldrenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
                        worldrenderer.pos(-i - 1, -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                        worldrenderer.pos(-i - 1, 8.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
...
//                        }
    ...
    protected boolean canRenderName(T entity) {
        ...
        return Minecraft.isGuiEnabled() && (entity != this.renderManager.livingPlayer || (NameTag.TOGGLE && entity == Minecraft.getMinecraft().thePlayer)) && !entity.isInvisibleToPlayer(entityplayersp) && entity.riddenByEntity == null;
    }
