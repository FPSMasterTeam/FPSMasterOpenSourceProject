...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn)
    {
        ...
        for (NetworkPlayerInfo networkplayerinfo : list)
        {
            ...
            if (CheatDetection.cheaters.containsKey(networkplayerinfo.getGameProfile().getId())) {
                if (CheatDetection.get(networkplayerinfo.getGameProfile().getId()).hacks) {
                    k = this.mc.fontRendererObj.getStringWidth("\247c\247l[CHEATER] \247r" + this.getPlayerName(networkplayerinfo));
                }
            }
            i = Math.max(i, k);
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                if (networkplayerinfo1.getGameType() == WorldSettings.GameType.SPECTATOR)
                {
                    ...
                    if (ClientSettings.hideTabName.getValue()) {
                        s1 = "\247k" + s1 + "\247r";
                    }
                    if (CheatDetection.cheaters.containsKey(networkplayerinfo1.getGameProfile().getId())) {
                        if (CheatDetection.get(networkplayerinfo1.getGameProfile().getId()).hacks) {
                            s1 = "\247c\247l[CHEATER] \247r" + s1;
                        }
                    }
