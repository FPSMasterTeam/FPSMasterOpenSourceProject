    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        if (this.visible) {
            ...
            if (this.getHoverState(this.hovered) == 2) {
                if (ClientSettings.betterButton.getValue()) {
                    anim = animationUtils.animate(255, anim, 0.2f,false);
                } else {
                    anim = 255;
                }
                ...
                GlStateManager.color(1.0F, 1.0F, 1.0F, Math.min(anim / 255f, 255));
                this.drawTexturedModalRect(this.xPosition, this.yPosition, 0, 46 + i * 20, this.width / 2, this.height);
                this.drawTexturedModalRect(this.xPosition + this.width / 2, this.yPosition, 200 - this.width / 2, 46 + i * 20, this.width / 2, this.height);
                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            } else {
                if (ClientSettings.betterButton.getValue()) {
                    anim = animationUtils.animate(0, anim, 0.2f,false);
                } else {
                    anim = 0;
                }
                this.drawTexturedModalRect(this.xPosition, this.yPosition, 0, 46 + i * 20, this.width / 2, this.height);
                ...
                GlStateManager.color(1.0F, 1.0F, 1.0F, Math.min(anim / 255f, 255));
                this.drawTexturedModalRect(this.xPosition, this.yPosition, 0, 46 + (i + 1) * 20, this.width / 2, this.height);
                this.drawTexturedModalRect(this.xPosition + this.width / 2, this.yPosition, 200 - this.width / 2, 46 + (i + 1) * 20, this.width / 2, this.height);
