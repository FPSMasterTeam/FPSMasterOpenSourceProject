...
public class TileEntityHopper extends TileEntityLockable implements IHopper, ITickable
{
    ...
    public static boolean captureDroppedItems(IHopper p_145891_0_)
    {
        if (BetterFpsConfig.getConfig().fastHopper) {
            ...
            TileEntityHopper hopperTE = p_145891_0_.getClass() == TileEntityHopper.class ? (TileEntityHopper) p_145891_0_ : null;

            IInventory iinventory = hopperTE == null ? getHopperInventory(p_145891_0_) : hopperTE.topInventory;
            ...
            if(iinventory != null) {
                ...
                if(iinventory instanceof ISidedInventory) {
                    ...
                    for (int j : aint) {
                        if (pullItemFromSlot(p_145891_0_, iinventory, j, enumfacing)) return true;
                    }
                } else {
                    int j = iinventory.getSizeInventory();

                    for(int k = 0; k < j; ++k) {
                        if(pullItemFromSlot(p_145891_0_, iinventory, k, enumfacing)) return true;
                    }
                }
    ...
    {
        if (BetterFpsConfig.getConfig().fastHopper) return isOnTransferCooldown;
