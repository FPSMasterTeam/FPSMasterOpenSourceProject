...
public abstract class World implements IBlockAccess {
    ...
    public boolean destroyBlock(BlockPos pos, boolean dropBlock) {
        ...
        // entity should also function correctly on 1.8.x protocol, so no need for base version checks
    ...
    public void markBlockForUpdate(BlockPos pos) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.markBlockForUpdate(pos));
    }
    ...
    public boolean isNotBlockTickPending(BlockPos pos, Block blockType) {
        return true;
    }
    ...
    public void playSoundAtEntity(Entity entityIn, String name, float volume, float pitch) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.playSound(name, entityIn.posX, entityIn.posY, entityIn.posZ, volume, pitch));
    }
    ...
    public void playSoundToNearExcept(EntityPlayer player, String name, float volume, float pitch) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.playSoundToNearExcept(player, name, player.posX, player.posY, player.posZ, volume, pitch));
    }
    ...
    public void playSoundEffect(double x, double y, double z, String soundName, float volume, float pitch) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.playSound(soundName, x, y, z, volume, pitch));
    }
    ...
    public void playRecord(BlockPos pos, String name) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.playRecord(name, pos));
    }
    ...
    private void spawnParticle(int particleID, boolean p_175720_2_, double xCood, double yCoord, double zCoord, double xOffset, double yOffset, double zOffset, int... p_175720_15_) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.spawnParticle(particleID, p_175720_2_, xCood, yCoord, zCoord, xOffset, yOffset, zOffset, p_175720_15_));
    }
    ...
    protected void onEntityAdded(Entity entityIn) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.onEntityAdded(entityIn));
    }

    protected void onEntityRemoved(Entity entityIn) {
        this.worldAccesses.forEach(worldAccess -> worldAccess.onEntityRemoved(entityIn));
    }
    ...
    public List<AxisAlignedBB> getCollisionBoxes(Entity entityIn, AxisAlignedBB aabb) {
        List<AxisAlignedBB> list = Lists.newArrayList();
        this.func_191504_a(entityIn, aabb, list);
        if (entityIn != null) {
            List<Entity> list1 = this.getEntitiesWithinAABBExcludingEntity(entityIn, aabb.expandXyz(0.25D));

            for (Entity o : list1) {
                if (!entityIn.isRidingSameEntity(o)) {
                    AxisAlignedBB axisalignedbb = o.getCollisionBoundingBox();
                    if (axisalignedbb != null && axisalignedbb.intersectsWith(aabb)) {
                        list.add(axisalignedbb);
                    }

                    axisalignedbb = entityIn.getCollisionBox(o);
                    if (axisalignedbb != null && axisalignedbb.intersectsWith(aabb)) {
                        list.add(axisalignedbb);
                    }
                }
    ...
    private boolean func_191504_a(Entity p_191504_1_, AxisAlignedBB p_191504_2_, List p_191504_4_) {
                    ...
                    if ((!flag2 || !flag3) && this.isBlockLoaded(blockpos$pooledmutableblockpos.setPos(k1, 64, l1))) {
                        for(int i2 = k; i2 < l; ++i2) {
                            if (!flag2 && !flag3 || i2 != l - 1) {
                                boolean var24;
                                if (p_191504_1_ != null && flag == flag1) {
     ...
     */
    public boolean isNotAnyLiquid(AxisAlignedBB bb) {
        int i = MathHelper.floor_double(bb.minX);
        int j = MathHelper.floor_double(bb.maxX);
        int k = MathHelper.floor_double(bb.minY);
        int l = MathHelper.floor_double(bb.maxY);
