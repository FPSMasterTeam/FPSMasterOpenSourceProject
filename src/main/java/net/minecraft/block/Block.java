...
public class Block {
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end) {
        ...
        if (this.isVecOutsideYZBounds(vec3)) {
            vec3 = null;
        }

        if (this.isVecOutsideYZBounds(vec31)) {
            vec31 = null;
        }

        if (this.isVecOutsideXZBounds(vec32)) {
            vec32 = null;
        }

        if (this.isVecOutsideXZBounds(vec33)) {
            vec33 = null;
        }

        if (this.isVecOutsideXYBounds(vec34)) {
            vec34 = null;
        }

        if (this.isVecOutsideXYBounds(vec35)) {
            vec35 = null;
        }
     ...
     */
    private boolean isVecOutsideYZBounds(Vec3 point) {
     ...
     */
    private boolean isVecOutsideXZBounds(Vec3 point) {
     ...
     */
    private boolean isVecOutsideXYBounds(Vec3 point) {
    ...
    {
        if (!blockBox.equals(new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D)))
        {
