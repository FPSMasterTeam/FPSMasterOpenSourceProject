...
public class BlockPos extends Vec3i
{
    ...
    public static class MutableBlockPos extends BlockPos
    {
        ...
        public BlockPos.MutableBlockPos move(EnumFacing facing, int p_189534_2_) {
            return this.set(this.x + facing.getFrontOffsetX() * p_189534_2_, this.y + facing.getFrontOffsetY() * p_189534_2_, this.z + facing.getFrontOffsetZ() * p_189534_2_);
        }
    }
    ...
    public static final class PooledMutableBlockPos extends MutableBlockPos {
        ...
        public static PooledMutableBlockPos retain(int xIn, int yIn, int zIn) {
            synchronized (POOL) {
                if (!POOL.isEmpty()) {
                    PooledMutableBlockPos blockpos$pooledmutableblockpos = (PooledMutableBlockPos) POOL.remove(POOL.size() - 1);
        ...
        public PooledMutableBlockPos setPos(int xIn, int yIn, int zIn) {
            if (this.released) {
                LogManager.getLogger().error("PooledMutableBlockPosition modified after it was released.", new Throwable());
                this.released = false;
            }
        ...
        public PooledMutableBlockPos setPos(Entity entityIn) {
            return (PooledMutableBlockPos) super.set(entityIn.posX, entityIn.posY, entityIn.posZ);
        }
        ...
        public PooledMutableBlockPos setPos(Vec3i vec) {
            return (PooledMutableBlockPos) super.set(vec.getX(), vec.getY(), vec.getZ());
        }

        public PooledMutableBlockPos move(EnumFacing facing) {
            return (PooledMutableBlockPos) super.move(facing, 1);
        }
