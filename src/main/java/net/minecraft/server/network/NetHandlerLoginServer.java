        ...
        else
        {
            ...
            (new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet())
            {
                public void run()
                {
                    ...
                    MinecraftSessionService minecraftSessionService = NetHandlerLoginServer.this.server.getMinecraftSessionService();
                    try {
                        NetHandlerLoginServer.this.loginGameProfile = (GameProfile) minecraftSessionService.getClass().getMethod("hasJoinedServer", GameProfile.class, String.class).invoke(minecraftSessionService.getClass(), new GameProfile(null, gameprofile.getName()), s);
                    } catch (InvocationTargetException | IllegalAccessException e) {
                        e.printStackTrace();
                    } catch (NoSuchMethodException e) {
                        System.out.println("NetEase Client Launcher detected!");
                        try {
                            NetHandlerLoginServer.this.loginGameProfile = (GameProfile) minecraftSessionService.getClass().getMethod("hasJoinedServer", GameProfile.class, String.class, InetAddress.class).invoke(minecraftSessionService.getClass(), new GameProfile(null, gameprofile.getName()), s, InetAddress.getByName("127.0.0.1"));
                        } catch (UnknownHostException | InvocationTargetException | IllegalAccessException | NoSuchMethodException ex) {
                            ex.printStackTrace();
                        }
                    }
