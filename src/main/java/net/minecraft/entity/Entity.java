...
public abstract class Entity implements ICommandSender {
    ...
    private boolean isLiquidPresentInAABB(AxisAlignedBB bb) {
        return this.worldObj.getCollidingBoundingBoxes(this, bb).isEmpty() && this.worldObj.isNotAnyLiquid(bb);
    }

    private static void checkBlockCollision(Entity entity) {
        try {
    ...
    /**
     * Tries to move the entity by the passed in displacement. Args: x, y, z
     */
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                AxisAlignedBB uniqueBoundingBox;
                double uniqueY, uniqueX, uniqueZ, doubleY, doubleX, doubleZ;
                uniqueY = y;

                for (AxisAlignedBB axisalignedbb6 : list) {
                    uniqueY = axisalignedbb6.calculateYOffset(axisalignedbb5, uniqueY);
                }

                axisalignedbb4 = axisalignedbb4.offset(0.0D, uniqueY, 0.0D);
                uniqueX = d3;

                for (AxisAlignedBB axisalignedbb7 : list) {
                    uniqueX = axisalignedbb7.calculateXOffset(axisalignedbb4, uniqueX);
                }

                axisalignedbb4 = axisalignedbb4.offset(uniqueX, 0.0D, 0.0D);
                uniqueZ = d5;

                for (AxisAlignedBB axisalignedbb8 : list) {
                    uniqueZ = axisalignedbb8.calculateZOffset(axisalignedbb4, uniqueZ);
                }

                axisalignedbb4 = axisalignedbb4.offset(0.0D, 0.0D, uniqueZ);
                uniqueBoundingBox = this.getEntityBoundingBox();
                doubleY = y;

                for (AxisAlignedBB axisalignedbb9 : list) {
                    doubleY = axisalignedbb9.calculateYOffset(uniqueBoundingBox, doubleY);
                }

                uniqueBoundingBox = uniqueBoundingBox.offset(0.0D, doubleY, 0.0D);
                doubleX = d3;

                for (AxisAlignedBB axisalignedbb10 : list) {
                    doubleX = axisalignedbb10.calculateXOffset(uniqueBoundingBox, doubleX);
                }

                uniqueBoundingBox = uniqueBoundingBox.offset(doubleX, 0.0D, 0.0D);
                doubleZ = d5;

                for (AxisAlignedBB axisalignedbb11 : list) {
                    doubleZ = axisalignedbb11.calculateZOffset(uniqueBoundingBox, doubleZ);
                }

                uniqueBoundingBox = uniqueBoundingBox.offset(0.0D, 0.0D, doubleZ);
                double d20 = uniqueX * uniqueX + uniqueZ * uniqueZ;
                double d10 = doubleX * doubleX + doubleZ * doubleZ;
                ...
                if (d20 > d10) {
                    ...
                    z = doubleZ;
                    y = -doubleY;
                    this.setEntityBoundingBox(uniqueBoundingBox);
                }
    ...
    public void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        if (onGroundIn) {
    ...
    public float getDistanceToEntityXZ(Entity entityIn) {
        ...
        return MathHelper.sqrt_float(f * f + f1 * f1);
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        if (list.isEmpty() && this.worldObj.isBlockNotFullCube(blockpos)) {
            return false;
        } else {
            int i = 3;
            double d3 = 9999.0D;

            if (this.worldObj.isBlockNotFullCube(blockpos.west()) && d0 < d3) {
                d3 = d0;
                i = 0;
            }

            if (this.worldObj.isBlockNotFullCube(blockpos.east()) && 1.0D - d0 < d3) {
                d3 = 1.0D - d0;
                i = 1;
            }

            if (this.worldObj.isBlockNotFullCube(blockpos.up()) && 1.0D - d1 < d3) {
                d3 = 1.0D - d1;
                i = 3;
            }

            if (this.worldObj.isBlockNotFullCube(blockpos.north()) && d2 < d3) {
                d3 = d2;
                i = 4;
            }

            if (this.worldObj.isBlockNotFullCube(blockpos.south()) && 1.0D - d2 < d3) {
                d3 = 1.0D - d2;
                i = 5;
            }
