...
import top.fpsmaster.event.events.EventStoppable;
...
public final class EventManager {
    ...
    public static void unregister(Object object, Method method) {
        MethodData toRemove = null;
        for (List<MethodData> methods : REGISTRY_MAP.values()) {
            for (MethodData methodData : methods) {
                if (methodData.target == method) {
                    toRemove = methodData;
                }
            }
        }
        MethodData finalToRemove = toRemove;
        REGISTRY_MAP.values().forEach(methodData -> methodData.remove(finalToRemove));
